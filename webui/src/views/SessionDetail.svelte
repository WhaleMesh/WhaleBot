<script>
  ...
  function chatBubbleClass(role) {
    ...
    if (role === 'system') return 'chat-bubble chat-bubble-ghost text-base-content/80 italic rounded-2xl';
  ...
  function formatCountdown(sec) {
    const loc = $locale;
    if (sec == null || sec < 0) return translate(loc, 'common.emDash');
    if (sec === 0) return translate(loc, 'common.minutesN', { n: '0' });
...
>
  <div class="flex flex-wrap items-center gap-2 pt-1">
    ...
    <h1 class="min-w-0 flex-1 text-xl font-bold tracking-tight sm:text-2xl">
      {$_('sessionDetail.titlePrefix')}
      <span class="wb-mono text-base font-normal text-neutral-content/50">{sessionId}</span>
    </h1>
  </div>
  ...
  <div class="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
    ...
    <div class="wb-stat-pill">
      ...
      <div class="mt-0.5 wb-mono text-sm font-medium text-base-content">{formatDateTime24(session?.created_at)}</div>
    </div>
    <div class="wb-stat-pill">
      <div class="text-[0.65rem] font-medium uppercase tracking-wide text-base-content/60">
        {$_('sessionDetail.updated')}
      </div>
      <div class="mt-0.5 wb-mono text-sm font-medium text-base-content">{formatDateTime24(session?.updated_at)}</div>
    </div>
    <div class="wb-stat-pill">
      <div class="text-[0.65rem] font-medium uppercase tracking-wide text-base-content/60">
...
<div class="wb-surface mb-4 !py-5">
  <div class="flex flex-col gap-3">
    <h2 class="text-lg font-semibold text-base-content">{$_('sessionDetail.runtimeTimeline')}</h2>
    {#if initialLoad}
      <div class="skeleton h-24 w-full"></div>
    {:else if sessionTraceEvents.length > 0}
      <div class="flex max-h-[34vh] flex-col gap-2 overflow-y-auto pr-1">
        {#each sessionTraceEvents.slice(0, 120) as evt}
          <details class="trace-item rounded-lg border border-base-300 bg-base-100 open:ring-1 open:ring-base-300">
            ...
            >
              <span class="wb-mono shrink-0 text-base-content/50">{formatDateTime24(evt.time)}</span>
    ...
    {:else}
      <p class="text-base text-base-content/60">{$_('sessionDetail.traceEmpty')}</p>
    {/if}
  </div>
</div>

<div class="w-full max-w-full pb-8">
  {#if initialLoad}
    ...
    {#each session.messages || [] as m}
      ...
      <div class="chat w-full {chatAlignClass(m.role)}">
        <div class="chat-header flex flex-wrap items-baseline gap-x-2 gap-y-0.5 px-1 text-[0.7rem] text-base-content/60">
          ...
          <span class="wb-mono">{formatDateTime24(m.timestamp)}</span>
          <span>
  ...
  {:else}
    <p class="py-8 text-center text-base-content/60">{$_('sessionDetail.unavailable')}</p>
  {/if}
</div>
