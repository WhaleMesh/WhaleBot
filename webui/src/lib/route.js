...
// Minimal hash router:
// { name: 'overview' | 'components' | 'sessions' | 'session' | 'tools' | 'tool' | 'logger' | 'skills' | 'llm' | 'adapter', params: { id? } }
...
function parseHash() {
  ...
  if (parts[0] === 'llm') {
    if (parts[1]) {
      return { name: 'llm', params: { id: decodeURIComponent(parts[1]) } };
    }
    return { name: 'llm', params: {} };
  }
  if (parts[0] === 'adapter') {
    if (parts[1]) {
      return { name: 'adapter', params: { id: decodeURIComponent(parts[1]) } };
    }
    return { name: 'adapter', params: {} };
  }
  if (parts[0] === 'skills') {
    if (parts[1]) {
      return { name: 'skills', params: { id: decodeURIComponent(parts[1]) } };
    }
  ...
  if (parts[0] === 'overview' || parts[0] === 'components' || parts[0] === 'sessions' || parts[0] === 'tools' || parts[0] === 'logger' || parts[0] === 'benchmark') {
...
function routeToHash(name, params = {}) {
  ...
  if (name === 'overview' || name === 'components' || name === 'sessions' || name === 'tools' || name === 'logger' || name === 'skills' || name === 'secrets' || name === 'llm' || name === 'adapter' || name === 'benchmark') {
    return `#/${name}`;
  }
  return '#/overview';
}
