...
// Minimal hash router:
// { name: 'overview' | 'components' | 'sessions' | 'session' | 'tools' | 'tool' | 'logger' | 'llm', params: { id? } }
...
function routeToHash(name, params = {}) {
  ...
  if (name === 'overview' || name === 'components' || name === 'sessions' || name === 'tools' || name === 'logger' || name === 'llm') {
    return `#/${name}`;
  }
  return '#/overview';
}
