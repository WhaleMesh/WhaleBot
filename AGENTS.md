...
- `runtime`
 ...
 - note: the system prompt is intentionally short (small-model friendly) and contains a terminology line mapping user phrasing (「用户容器 / userdocker / 用户 Docker」) to the `docker_*` tools (e.g. delete = `docker_lifecycle action=remove`); container-selection process knowledge lives in the tool descriptions — reuse an existing container (matched by its `purpose` from `action=list`) before creating from a framework image, and only pull external images after `estimate_image_pull` + explicit user approval; agents record installed environments in each container's `/workspace/.whalebot/NOTES.md`. Long installs/builds use `action=exec` `async=true` + `action=exec_status` polling. **Build-then-run provenance**: binaries deployed from a disposable build container into a run container must carry `src.tar.gz` (unextracted) + `/workspace/.whalebot/BUILD.md` (build image/commands/toolchain/artifact/date) copied via `copy_file` **before** the build container is removed; default one dedicated run container per service (no ask-to-consolidate)
...
- `skills`
  ...
  - note: on first start (**empty `packages/`**), seeds **`whaletrue-best-practices/`** from embedded defaults; existing package dirs are not modified
...
- `webui`
  ...
  - note: **brand**: sidebar (and sign-in card) **brand icon** links to `https://github.com/WhaleTrue/WhaleBot` in a **new browser tab** (`webui/src/lib/brandUrls.js`, `WbBrandIcon.svelte`, `App.svelte`).
  - note: sidebar **footer** (below language + account): **Powered by WhaleTrue** with **WhaleTrue** linking to `https://github.com/WhaleTrue` (new tab); copy is i18n-driven (`layout.poweredByBefore` / `whaleTrue` / `layout.poweredByAfter`).
...
- LLM invoke timeout:
 - `LLM_INVOKE_TIMEOUT_SEC` (llm-openai per-invoke upstream budget, default 60; raise for slow local models; context also covers a 3-minute continuous-429 retry window)
