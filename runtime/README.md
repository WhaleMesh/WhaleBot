...
- Build-then-run provenance: when a binary built in one (disposable) container is deployed to run in another, the agent packs the source as `/workspace/src.tar.gz`, copies it plus the binary into the run container via `copy_file` (same node only), and writes `/workspace/.whalebot/BUILD.md` (build image, commands, toolchain version, artifact path, date) — all before removing the build container. `src.tar.gz` stays unextracted; upgrades start from a fresh build container. Default is one dedicated run container per service.
