...
func userDockerToolDefinitions() []map[string]any {
	return []map[string]any{
		...
		fnTool("docker_exec",
			...
			[]string{"action", "name"}),
		fnTool("docker_files",
			"Read and write files under a container's /workspace. action=copy_file copies any file (including binaries) from one container to another on the same node — use it instead of shell pipelines to move build outputs, e.g. {\"action\":\"copy_file\",\"name\":\"nodeA/build-1\",\"path\":\"/workspace/app\",\"to_name\":\"nodeA/run-1\",\"to_path\":\"/workspace/app\"}. Before reusing a container, read /workspace/.whalebot/NOTES.md (if present) to learn what is installed; after installing new tooling, append an update to NOTES.md via write_file. Deploy provenance: when a binary built in one container is deployed to run in another, pack the source in the build container (docker_exec: tar czf /workspace/src.tar.gz <srcdir>), copy_file BOTH the binary and src.tar.gz into the run container, and write /workspace/.whalebot/BUILD.md there (source layout, build image + commands, toolchain version, artifact path, date) — all BEFORE removing the build container. Keep src.tar.gz unextracted; to upgrade later, copy src.tar.gz into a fresh build container and follow BUILD.md. Note copy_file only works between containers on the same node. Example: {\"action\":\"write_file\",\"name\":\"c1\",\"path\":\"/workspace/main.go\",\"content\":\"package main...\"}",
			map[string]any{
